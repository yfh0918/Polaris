...
public class FilterTypeHelper {
    ...
    static {
    	...
    	for (FilterType filterType : FilterType.values()) {
    		...
    		//先获取
    		loadPatters(filterType.name(), ConfHandlerProviderFactory.get(Type.EXT).get(filterType.getFileName()));
    		
    		//后监听
    		ConfHandlerProviderFactory.get(Type.EXT).listen(filterType.getFileName(), new ConfHandlerListener() {
    			@Override
