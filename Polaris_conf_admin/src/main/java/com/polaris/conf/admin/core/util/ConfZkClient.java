...
public class ConfZkClient implements Watcher {
	...
	public static Stat setPathDataByKey(String key, String data, boolean isWatch) {
		...
		try {
			...
			if (data != null) {
				return zooKeeper.setData(path, data.getBytes(com.polaris.comm.Constant.UTF_CODE),stat.getVersion());
			}
		} catch (Exception e) {
			logger.error(e);
		}
	...
	private static String getPathData(String path, boolean isWatch){
		try {
			...
			if (stat != null) {
				...
				if (resultData != null) {
					znodeValue = new String(resultData,com.polaris.comm.Constant.UTF_CODE);
				}
				return znodeValue;
			} else {
